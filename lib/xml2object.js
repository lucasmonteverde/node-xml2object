...
var xml2object = module.exports = function(elements, source, saxOptions) {
	...
	this.saxStream
		...
		/* .on("closecdata", function(name) {
			closeTag("$d");
		}); */
		.on('error', function (e) { // Rebroadcast the error and keep going
			self.emit('error', e);
		...
		.on('end', function() { // Rebroadcast the end of the file read
			self.emit("end");
		});
};
